...
def loadDataset( outputDatasetFile, listOfdict, statesList):
	with io.open("DiabetesBin.txt") as labelFile, io.open("LabelList.txt") as labelListFile:
		...
		for itrx in range(51):
			...
			for itry in range(51):
				if itrx != itry:
					data = {}
					data["tweet"] = listOfdict[itry]
					data["label"] = labels[labelIndx[statesList[itry]]]
					newList.append(data)
				else:
					data = {}
					data["tweet"] = listOfdict[itry]
			...
			# 	writer.close()
			with jsonlines.open(str("StateTwtTest") + str(itrx) + str(".jsonl"), mode='w') as writer:
				writer.write_all(testList)
				writer.close()
			print("Done")
		...
		# print(jstr)
def MakeStateDataSet(inputfile ):
	with io.open(inputfile, encoding="utf8") as dataset:
		listOfTweetsForStates = [[] for i in range(51)]
		print(listOfTweetsForStates)
		stateNumber = {}
		idx = 0
		states = []
		for line in dataset:
			...
			#print(tweet)
			#return
			if stateName not in stateNumber:
				stateNumber[stateName] = idx
				idx += 1
				states.append(stateName)
				listOfTweetsForStates[idx-1].append(tweet)
			else:
				stateIdx = stateNumber[stateName]
				listOfTweetsForStates[stateIdx].append(tweet)

		for idx in range(51):
			listOfTweetsForStates[idx] = ' '.join(listOfTweetsForStates[idx])
	#print(listOfTweetsForStates[0])
	loadDataset("StateTwt", listOfTweetsForStates, states)


		
		# with jsonlines.open("AllStateTweetByLines.jsonl", mode='w') as writer:
		# 	writer.write_all(listOfTweetsForStates)
		# 	writer.close()
...
def main(argv):
	...
	if( option == 1):
		...
		MakeStateDataSet("TweetsWithStatesAll.txt")
	elif(option == 4):
		print("Testing HPC")
